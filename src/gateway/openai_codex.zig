...
fn credentialSourceMatches(source: ?types.CredentialSource, comptime tag: std.meta.Tag(types.CredentialSource)) bool {
...
fn streamCompletionCore(alloc: Allocator, request: stream_provider.Request) !stream_provider.Result {
    if (request.cancel_flag.load(.seq_cst)) return error.Cancelled;
    if (!credentialSourceMatches(request.credential_source, .chatgpt_subscription)) {
