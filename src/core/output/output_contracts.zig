...
fn gatewayProviderConnected(auth: auth_runtime.StatusSnapshot) bool {
    ...
    if (auth.gateway_connected) return true;
    return !isSubscriptionSource(source);
}

fn chatGptProviderConnected(auth: auth_runtime.StatusSnapshot) bool {
    return auth.chatgpt_connected or optionalSourceIs(auth.active_source, .chatgpt_subscription);
}

fn grokProviderConnected(auth: auth_runtime.StatusSnapshot) bool {
    return auth.grok_connected or optionalSourceIs(auth.active_source, .grok_subscription);
}

fn isSubscriptionSource(source: types.CredentialSource) bool {
...
fn optionalSourceIs(source: ?types.CredentialSource, comptime tag: std.meta.Tag(types.CredentialSource)) bool {
    const unwrapped = source orelse return false;
    return std.meta.activeTag(unwrapped) == tag;
}
...
pub const ModelListSnapshot = struct {
    ...
    fn emptyCatalogProviderName(self: ModelListSnapshot) []const u8 {
        return switch (self.provider) {
            .gateway => "gateway",
            inline else => |tag| comptime model_provider.label(tag),
        };
    }
