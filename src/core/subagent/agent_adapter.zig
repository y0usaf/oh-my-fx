...
pub const ProviderRoutes = struct {
    routes: std.EnumMap(model_provider.ProviderId, ProviderRoute) =
        std.EnumMap(model_provider.ProviderId, ProviderRoute).init(.{}),
    ...
    pub fn put(
        ...
        self.routes.put(provider, route);
    }

    /// Missing routes degrade to an unavailable stream with no reviewer so a
    /// host that only wires the fx-native trio still links and runs.
    pub fn select(self: ProviderRoutes, provider: model_provider.ProviderId) ProviderRoute {
        return self.routes.get(provider) orelse .{
...
test "provider routes select independent streams and reviewers" {
    ...
    routes.put(.gateway, .{ .agent_stream_provider = gateway_stream, .permission_reviewer_provider = gateway_reviewer });
    routes.put(.codex, .{ .agent_stream_provider = codex_stream, .permission_reviewer_provider = codex_reviewer });
    routes.put(.grok, .{ .agent_stream_provider = grok_stream, .permission_reviewer_provider = grok_reviewer });
