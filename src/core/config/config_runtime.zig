...
pub const Settings = struct {
    ...
    pub fn deinit(self: *Settings, alloc: Allocator) void {
        ...
        if (self.openrouter_model) |value| alloc.free(value);
...
pub const ConfigSources = struct {
    ...
    openrouter_model: ConfigSource = .compiled_default,
...
fn loadMergedSettingsDetailedWithOptionalHome(
    ...
    // Single active pair: a legacy per-provider model key (`codex_model`,
    // `grok_model`, `openrouter_model`) migrates into `model` when it belongs
    // to the active provider and no explicit model is set. Source tracking
    // follows the adopted key.
    migrateLegacyModelKey(&settings, &sources);
...
fn updateConfigSources(sources: *ConfigSources, settings: Settings, source: ConfigSource) void {
    ...
    if (settings.openrouter_model != null) sources.openrouter_model = source;
...
fn migrateLegacyModelKey(settings: *Settings, sources: *ConfigSources) void {
    const provider = settings.provider orelse .gateway;
    const legacy_value: ?[]u8 = switch (provider) {
        .codex => settings.codex_model,
        .grok => settings.grok_model,
        .openrouter => settings.openrouter_model,
        else => null,
    };
    const value = legacy_value orelse return;
    if (settings.model == null) {
        settings.model = value;
        sources.model = switch (provider) {
            .codex => sources.codex_model,
            .grok => sources.grok_model,
            .openrouter => sources.openrouter_model,
            else => sources.model,
        };
    }
    // Ownership moved into `model` when adopted; clear the slot either way so
    // deinit never double-frees and writers stop persisting the legacy key.
    switch (provider) {
        .codex => settings.codex_model = null,
        .grok => settings.grok_model = null,
        .openrouter => settings.openrouter_model = null,
        else => {},
    }
}
...
fn parseProfileOnlyFields(
    ...
    if (root.object.get("openrouter_model")) |model_value| {
        if (model_value != .string) return error.InvalidOpenRouterModelType;
        settings_store.validateModel(model_value.string) catch return error.InvalidOpenRouterModelValue;
        settings.openrouter_model = try alloc.dupe(u8, model_value.string);
    }
...
fn mergeSettings(target: *Settings, incoming: *Settings, alloc: Allocator) void {
    ...
    if (incoming.openrouter_model) |value| {
        if (target.openrouter_model) |current| alloc.free(current);
        target.openrouter_model = value;
        incoming.openrouter_model = null;
    }
    if (incoming.permission_mode) |value| target.permission_mode = value;
