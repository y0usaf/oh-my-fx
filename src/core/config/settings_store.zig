...
pub const UserSettingsPatch = struct {
    ...
    // Single active pair semantics: the active provider's model id lives in
    // `model`; per-provider keys are read-only migration inputs.
...
test "provider patch stores the active model and drops migrated legacy keys" {
    const alloc = std.testing.allocator;
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    ...
    var root = try std.json.parseFromSliceLeaky(
        std.json.Value,
        arena.allocator(),
        "{\"model\":\"gateway/model\",\"codex_model\":\"gpt-5.4-mini\",\"grok_model\":\"grok-4\"}",
        .{},
    );
    ...
    try std.testing.expectEqualStrings("gpt-5.4-mini", root.object.get("model").?.string);
    try std.testing.expect(!root.object.contains("codex_model"));
    try std.testing.expect(!root.object.contains("grok_model"));
    try std.testing.expect(!root.object.contains("openrouter_model"));
...
fn applyUserPatchToRoot(
    ...
    if (patch.provider) |value| application.changed = try putString(arena, &root.object, "provider", model_provider.providerSlug(value)) or application.changed;
    // Writing an active pair drops migrated legacy keys so the file converges
    // on `model` instead of accumulating stale per-provider entries.
    if (patch.model != null or patch.provider != null) {
        for ([_][]const u8{ "codex_model", "grok_model", "openrouter_model" }) |key| {
            if (root.object.orderedRemove(key)) application.changed = true;
        }
    }
...
fn validateKnownSettingsObject(
    ...
    if (object.get("openrouter_model")) |value| {
        if (value != .string) return error.InvalidSettingsFormat;
        try validateModel(value.string);
    }
