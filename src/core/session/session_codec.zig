...
fn writeState(writer: *std.Io.Writer, state: DurableSessionState) !void {
    ...
    try writeJsonString(writer, model_provider.providerSlug(state.preferences.provider));
...
pub fn writeRecoveryCheckpoint(writer: *std.Io.Writer, checkpoint: RecoveryCheckpoint) !void {
    ...
    try writeJsonString(writer, model_provider.providerSlug(checkpoint.route_provider));
