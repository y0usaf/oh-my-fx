...
pub fn runSubagentChild(
    ...
    return subagent_agent_adapter.run(.{
        ...
        .provider_routes = buildProviderRoutes(state),
...
fn buildProviderRoutes(state: *server.ServerState) subagent_agent_adapter.ProviderRoutes {
    ...
    inline for (std.meta.tags(model_provider.ProviderId)) |id| {
        const reviewer: ?permission_auto_classifier.Provider = switch (id) {
            .gateway => state.cfg.permission_reviewer_provider,
            .codex => state.cfg.codex_permission_reviewer_provider,
            .grok => state.cfg.grok_permission_reviewer_provider,
            else => null,
        };
        routes.put(id, .{
            .agent_stream_provider = server.streamProviderFor(state, id),
